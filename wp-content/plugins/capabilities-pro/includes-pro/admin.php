...
class CustomStatusCapsUI {
    ...
    function drawUI ($args = []) {
        ...
        if (Pro::customStatusPostMetaPermissions()) {
            if ($attributes = \PublishPress\Permissions\Statuses::instance()::attributes()) {
        ...
        . __('Custom Statuses', 'capabilities-pro-by-publishpress') 
        . '</a></h3>';
        ...
        $cap_tips = array( 
            'set' =>            __( 'Can assign this status (for selected post types)', 'capabilities-pro-by-publishpress' ),
            'edit' =>           __( 'Can edit posts of this status (for selected post types)', 'capabilities-pro-by-publishpress' ),
            'edit_others' =>    __( 'Can edit other\'s posts of this status (for selected post types)', 'capabilities-pro-by-publishpress' ),
            'delete' =>         __( 'Can delete posts of this status (for selected post types)', 'capabilities-pro-by-publishpress' ),
            'delete_others' =>  __( 'Can delete other\'s posts of this status (for selected post types)', 'capabilities-pro-by-publishpress' ),
        );
            ...
            foreach( $cap_property_prefixes as $prefix ) {
                ...
                if ($is_post_meta_status = !empty($postmeta_statuses[$status])) {
                        ...
                        && (($prefix != 'set') || !empty($type_obj->cap->set_posts_status))
                            ...
                            if ($cap_name) {
                                ...
                                if ($is_administrator || current_user_can($cap_name)) {
                                    if (!empty($pp_metagroup_caps[$cap_name])) {
                                        $title_text = sprintf( __( '%s: assigned by Permission Group', 'capabilities-pro-by-publishpress' ), $cap_name );
                                    } else {
                                        $title_text = $cap_name;
                                    }
                                    ...
                                    $checkbox = '<input type="checkbox" title="' . $title_text . '" name="caps[' . $cap_name . ']" value="1" ' . $checked . $disabled . ' /> ' . $type_obj->label;
                            ...
                            $title = 'title="' . sprintf( __('shared capability: %s', 'capabilities-pro-by-publishpress'), esc_attr($type_obj->cap->$prop)) . '"';
                        }
                    ...
                    if ('set' == $prefix) {
                        ...
                        echo "<div class='types'><a href='$url' target='_blank'>" . __('Post Types...', 'capabilities-pro-by-publishpress') . "</a></div>";
                    }
                    ...
                    if ($is_administrator || current_user_can($cap_name)) {
                        ...
                        $td_class = ( $td_classes ) ? 'class="' . implode(' ', $td_classes) . '"' : '';
                        $status_col_ui .= '<input type="checkbox" ' . $td_class . 'title="' . $title_text . '" name="caps[' . $cap_name . ']" value="1" ' . $checked . $disabled . ' /> ';
        ...
        <?php if (!empty($display_row)):?>
            ...
            <p>
            <a href="<?php echo admin_url('admin.php?page=presspermit-statuses&attrib_type=moderation');?>" target="_blank"><?php _e('Enable / Disable Custom Permissions', 'capabilities-pro-by-publishpress');?></a>
            </p>
            </td>
        <?php elseif($do_postmeta_permissions && empty($postmeta_statuses)):?>
            <td class="cme-status-footer cme-custom-status-hints" colspan="2">
            <p>
            <a href="<?php echo admin_url('admin.php?page=presspermit-statuses&attrib_type=moderation');?>" target="_blank"><?php _e('Customize Permissions', 'capabilities-pro-by-publishpress');?></a>
            </p>
            </td>
        <?php else:?>
            <td colspan="2">
            ...
            <?php
            printf(__('%sAdvanced status control...%s', 'capabilities-pro-by-publishpress'), "<a href='#pp-advanced-status-control' target='_blank'>", '</a>');
            ?>
            </div>
            <div class='pp-status-control-notice' style='display:none'>
            <?php
            $url = admin_url('admin.php?page=presspermit-settings');
            printf(__('For more control, <br />enable the PublishPress Permissions <br />%sStatus Control module%s', 'capabilities-pro-by-publishpress'), "<a href='$url' target='_blank'>", '</a>' );?>
            </div>
            </div>
            <?php endif;?>
            </td>
