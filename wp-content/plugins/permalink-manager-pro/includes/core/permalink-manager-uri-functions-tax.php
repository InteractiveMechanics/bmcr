	...
	function custom_tax_permalinks($permalink, $term) {
		...
		if(isset($permalink_manager_uris["tax-{$term_id}"])) {
			...
			if(!empty($permalink_manager_options['general']['decode_uris'])) {
				$permalink .= urldecode("/{$permalink_manager_uris["tax-{$term_id}"]}");
			} else {
		...
		return apply_filters('permalink_manager_filter_final_term_permalink', user_trailingslashit($permalink), $term, $old_permalink);
	}
