...
class Permalink_Manager_Pro_Functions extends Permalink_Manager_Class {
	...
	public function check_for_updates($flush_exp_date = false) {
		...
		if(!empty($_POST['licence']['licence_key']) || (!empty($_REQUEST['action']) && $_REQUEST['action'] == 'pm_get_exp_date')) {
	...
	public static function get_expiration_date($basic_check = false, $empty_if_valid = false) {
		...
		// Key expired
			...
			$expiration_info = sprintf(__('Your Permalink Manager Pro licence key expired! To restore access to plugin updates & technical support please go to <a href="%s" target="_blank">this page</a>.', 'permalink-manager'), 'https://permalinkmanager.pro/docs/billing/how-to-renew-the-license-key/');
			$expired = 1;
		}
		// Lifetime license key
		else if(date("Y", intval($exp_date)) > 2050) {
			$expiration_info = __('You own a lifetime licence key.', 'permalink-manager');
			$expired = 0;
		} else if($exp_date) {
			$expiration_info = sprintf(__('Your licence key is valid until %s.<br />To prolong it please go to <a href="%s" target="_blank">this page</a> for more information.', 'permalink-manager'), date_i18n(get_option('date_format'), $exp_date), 'https://permalinkmanager.pro/docs/billing/how-to-renew-the-license-key/');
			$expired = 0;
		} else {
			$expiration_info = sprintf(__('Please paste the licence key to access all Permalink Manager Pro updates & features <a href="%s" target="_blank">on this page</a>.', 'permalink-manager'), $settings_page_url);
			$expired = 2;
