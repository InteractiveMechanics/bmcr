<?php
/**
 * PublishPress Multiple Authors Pro plugin bootstrap file.
 *
 ...
 * Plugin Name: PublishPress Multiple Authors Pro
    ...
    if (version_compare($presspermitVersions['version'], '2.7.23', '<=')) {
        if ( ! strpos(urldecode($_SERVER['REQUEST_URI']), 'deactivate')) {
            add_action('all_admin_notices', function () {
                $msg = sprintf(
                    __('Error: %1$s is not compatible with PressPermit Pro v2.7.23 or prior. Sorry, but we recommend updating PressPermit.', 'publishpress-multiple-authors'),
...
if ( ! defined('PP_MULTIPLE_AUTHORS_DEFINED')) {
    ...
    if (defined('WP_CLI') && WP_CLI) {
        WP_CLI::add_command('publishpress-multiple-authors', 'MultipleAuthors\\WP_Cli');
    }
    ...
    register_activation_hook(PP_MULTIPLE_AUTHORS_FILE, function () {
        ...
        if (('plugins.php' == $pagenow) && ! strpos(urldecode($_SERVER['REQUEST_URI']), 'deactivate')) {
            add_action('all_admin_notices', function () {
                $msg = sprintf(
                    ...
                    'PublishPress Multiple Authors Pro',
                    dirname(PP_MULTIPLE_AUTHORS_BASENAME)
                );
