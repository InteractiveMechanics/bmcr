...
class Post_Editor {
	...
	public static function filter_manage_posts_columns( $columns ) {
		...
		foreach ( $columns as $key => $title ) {
			...
			if ( 'title' === $key ) {
				$new_columns['authors'] = __( 'Authors', 'publishpress-multiple-authors' );
			}
	...
	public static function action_add_meta_boxes_late() {
		...
		foreach ( Content_Model::get_author_supported_post_types() as $post_type ) {
			...
			add_meta_box( 'authors', __( 'Authors', 'publishpress-multiple-authors' ),
	...
	public static function render_authors_metabox() {
		...
		if ( empty( $authors ) ) {
            error_log( 'Function render_authors_metabox: Authors not found to render the metabox' );
        ...
        <ul class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
			...
			foreach ( $authors as $author ) {
				...
				echo self::get_rendered_author_partial( [
					...
					'avatar'       => get_avatar( $author->user_email, 20 ),
					'term'         => $term,
				] );
			}
			?>
