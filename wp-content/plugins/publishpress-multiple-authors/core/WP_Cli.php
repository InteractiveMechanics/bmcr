<?php
/**
 * PublishPress Multiple Authors commands for the WP-CLI framework
 *
 * @package wp-cli
 * @since   3.0
...
class WP_Cli extends WP_CLI_Command {
	...
	public function create_terms_for_posts() {
		...
		while ( $posts->post_count ) {
			...
			foreach ( $posts->posts as $single_post ) {
				...
				if ( ! empty( $terms ) ) {
					WP_CLI::line( "{$count}/{$posts->found_posts}) Skipping - Post #{$single_post->ID} '{$single_post->post_title}' already has these terms: " . implode( ', ',
	...
	public function assign_user_to_coauthor( $args, $assoc_args ) {
		...
		if ( ! $user ) {
			WP_CLI::error( __( 'Please specify a valid user_login', 'publishpress-multiple-authors' ) );
		}

		if ( ! $coauthor ) {
			WP_CLI::error( __( 'Please specify a valid co-author login', 'publishpress-multiple-authors' ) );
		}
		...
		foreach ( $posts as $post_id ) {
			...
			WP_CLI::line( sprintf( __( "Updating - Adding %s's byline to post #%d", 'publishpress-multiple-authors' ),
		...
		WP_CLI::success( sprintf( __( 'All done! %d posts were affected.', 'publishpress-multiple-authors' ),
			$affected ) );
