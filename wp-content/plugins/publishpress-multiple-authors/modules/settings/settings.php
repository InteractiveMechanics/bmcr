...
if ( ! class_exists('MA_Settings')) {
    class MA_Settings extends Module
    {
        ...
        public function __construct()
        {
            ...
            $args             = [
                'title'                => __('PublishPress Multiple Authors Pro', 'publishpress-multiple-authors'),
        ...
        public function action_admin_submenu()
        {
            ...
            add_submenu_page(
                ...
                esc_html__('PublishPress Settings', 'publishpress-multiple-authors'),
                esc_html__('Settings', 'publishpress-multiple-authors'),
                apply_filters('ppma_view_settings_cap', 'manage_options'),
                self::MENU_SLUG,
        ...
        {
            if ($this->is_whitelisted_settings_view()) {
                wp_enqueue_style('publishpress-settings-css', $this->module_url . 'lib/settings.css', false, PUBLISHPRESS_MULTIPLE_AUTHORS_VERSION);
            }
        }
        ...
        public function print_modules()
        {
            ...
            if (!count($legacyPlugin->modules)) {
                echo '<div class="message error">' . __('There are no PublishPress modules registered', 'publishpress-multiple-authors') . '</div>';
            } else {
        ...
        public function helper_option_custom_post_type($module, $post_types = [])
        {
            ...
            foreach ($post_types as $post_type => $title) {
                ...
                if (post_type_supports($post_type, $module->post_type_support)) {
                    echo '&nbsp&nbsp;&nbsp;<span class="description">' . sprintf(__('Disabled because add_post_type_support(\'%1$s\', \'%2$s\') is included in a loaded file.', 'publishpress-multiple-authors'), $post_type, $module->post_type_support) . '</span>';
                }
                echo '<br />';
            }
        }
