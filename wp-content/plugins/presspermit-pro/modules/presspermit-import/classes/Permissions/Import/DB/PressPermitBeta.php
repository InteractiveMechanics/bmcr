...
class PressPermitBeta extends \PublishPress\Permissions\Import\Importer
{
    ...
    function get_exception_id(&$stored_exceptions, $data, $restriction_id)
    {
        ...
        if (!$exception_id) {
            ...
            global $blog_id;
            $log_data = ['run_id' => $this->run_id, 'source_tbl' => $this->getTableCode($wpdb->pp_conditions), 'source_id' => $restriction_id, 'import_tbl' => $this->getTableCode($wpdb->ppc_exceptions), 'import_id' => $exception_id, 'site' => $blog_id];
            $wpdb->insert($wpdb->ppi_imported, $log_data);
