...
class UserSync
{
    ...
    function syncPostsToUsers($args = [])
    {
        ...
        foreach ($post_types as $post_type) {
            ...
            if (!$role) {
                $this->log[] = sprintf(__('%s: Skipping because no role is selected', 'ppsync'), $post_type_obj->label);
            ...
            */
            ...
            if ($pp->getOption('sync_posts_to_users_apply_permissions')) {
                ...
                if (empty($filtered_post_types[$post_type])) {
                    ...
                    $this->log[] = sprintf(
                        __('%s: Enabled PressPermit filtering (Core > Filtered Post Types)', 'ppsync'), 
                        $post_type_obj->label
                    );
                }
                ...
                if ($group_id = $pp->groups()->getMetagroup('wp_role', $role, ['cols' => 'id'])) {
                    ...
                    if (empty($supplemental_roles[$pp_role_name])) {
                        ...
                        $this->log[] = sprintf(
                            __('%1$s: Assigned supplemental %2$s Author role to [WP %3$s] group', 'ppsync'), 
                            $post_type_obj->label, 
                ...
                if ('post_title' == $post_field) {
                    ...
                    foreach (array_keys($matched_posts) as $post_title) {
                        ...
                        if (count($post_title_instances) > 1) {
                            $this->log[] = sprintf(
                                __('Ambiguous %1$s match (%2$s)', 'ppsync'), 
                                $post_type_obj->labels->singular_name, 
                                $post_title
                            );
                    ...
                    foreach ($matched_posts as $display_name => $post_id) {
                        if ($sync_user_id) {
                            ...
                            if (count($display_name_users) > 1) {
                                $this->log[] = sprintf(
                                    __('Ambiguous user match for %1$s (%2$s, post id %3$s', 'ppsync'), 
                                    $post_type_obj->labels->singular_name, 
                                    $display_name, 
                                    $post_id
                                );
            ...
            */
                ...
                if ('display_name' == $user_field) {
                    foreach (array_keys($unmatched_users) as $display_name) {
                        if ($sync_user_id) {
                            ...
                            if (count($display_name_users) > 1) {
                                $this->log[] = sprintf(
                                    __('Ambiguous user match for %1$s (%2$s)', 'ppsync'), 
                                    $post_type_obj->labels->singular_name, 
                                    $display_name
                                );
            ...
            if ($insert_count) $this->log[] = sprintf(
                __('%1$s: %2$s posts created', 'ppsync'), 
                $post_type_obj->label, 
                $insert_count
            );

            if ($update_count) $this->log[] = sprintf(
                __('%1$s: %2$s posts updated', 'ppsync'), 
                $post_type_obj->label, 
                $update_count
            );
            ...
            if ($role) {
                ...
                $this->log[] = sprintf(__('%1$s: Synchronization done for %2$s role.', 'ppsync'), $post_type_obj->label, $role_label);
            } else
                $this->log[] = sprintf(__('%s: Synchronization done.', 'ppsync'), $post_type_obj->label);
