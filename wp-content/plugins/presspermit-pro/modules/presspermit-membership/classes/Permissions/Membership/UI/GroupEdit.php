    ...
    function fltAgentsSelectionArgs( $args, $agent_type, $id_suffix ) {
        if ( 'member' == $id_suffix ) {
            ...
            $args['label_selections'] = sprintf( 
                __( 'Current Selections: <span style="float:right"><small>%1$s refresh %2$s</small></span>' ), 
                '<a href="' . esc_url($url) . '" title="' . $title . '">', 
                '</a>' 
            );
        }
    ...
    function fltAgentsSelectionAttribs( $return, $agent_type, $id_suffix, $stored_agent ) {
        if ( 'member' == $id_suffix ) {
            ...
            if ( $stored_agent->date_limited ) {
                ...
                $_title = sprintf( __( '%1$s (%2$s - %3$s)', 'ppm' ), $_title, $start_caption, $end_caption );
