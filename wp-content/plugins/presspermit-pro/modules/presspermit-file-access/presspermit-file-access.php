...
if (defined('PRESSPERMIT_FILEACCESS_FILE')) {
    ...
    add_action(
        ...
        function()
        {
            ...
            if (is_admin()) {
                @load_plugin_textdomain( 'ppff', '', dirname(plugin_basename(__FILE__)) . '/languages');
                
                $title = __('File Access', 'ppff');
            } else {
                $title = 'File Access';
            }
            ...
            if (did_action('presspermit_activate') || get_option('presspermit_activation')) {
