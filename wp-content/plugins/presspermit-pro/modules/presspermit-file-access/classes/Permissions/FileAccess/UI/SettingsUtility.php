...
class SettingsUtility
{
    ...
    <div class='wrap'>
        <table width="100%">
            <tr>
                <td width="90%">
                    <h2><?php _e('Attachments Utility', 'presspermit') ?></h2>
                    <?php
                ...
                <td>
                    <div class="submit" style="border:none;float:right;margin:0;">
                        <input type="submit" name="presspermit_submit" class="button-primary"
                            value="<?php _e('Update &raquo;', 'presspermit'); ?>"/>
                    </div>
                </td>
            </tr>
        </table>
    ...
    private static function attach_linked_uploads($echo = false)
    {
        ...
        foreach ($blog_ids as $id) {
            ...
            if (false === strpos($uploads['baseurl'], $site_url)) {
                if ($echo) {
                    _e('<strong>Note</strong>: Direct access to uploaded file attachments cannot be filtered because your WP_CONTENT_DIR is not in the WordPress branch.', 'presspermit');
                    echo '<br /><br />';
                    _e('The operation was terminated due to an invalid configuration.', 'presspermit');
                }
            ...
            if ($post_ids = $wpdb->get_col("SELECT ID FROM $wpdb->posts WHERE post_type IN ($post_type_in) ORDER BY post_type, post_title")) {
                ...
                if ($echo) {
                    printf(__("<strong>checking %s posts / pages...</strong>", 'presspermit'), $num_posts);
                    echo '<br /><br />';
                }
                    ...
                    foreach ($results as $row) {
                        ...
                        foreach (array_keys($tags) as $tag_type) {
                            foreach ($tags[$tag_type]['0'] as $found_tag) {
                                ...
                                if (false === strpos($file_url, $upload_path)) {
                                    if ($echo) {
                                        printf(__('<span class="pp-brown">skipping unfilterable file in %1$s "%2$s":</span> %3$s', 'presspermit'), __(ucwords($row->post_type)), $row->post_title, $file_url);
                                        echo '<br /><br />';
                                    }
                                ...
                                if (!file_exists(str_replace($upload_path, $upload_dir, $file_url))) {
                                    if ($echo) {
                                        printf(__('<span class="pp-red">skipping missing file in %1$s "%2$s":</span> %3$s', 'presspermit'), __(ucwords($row->post_type)), $row->post_title, $file_url);
                                        echo '<br /><br />';
                                    }
                        ...
                        foreach ($linked_uploads as $file_url => $caption) {
                            ...
                            if (!isset($stored_attachments[$row->ID][$unsuffixed_file_url])) {
                                ...
                                if ($echo) {
                                    printf(__('<span class="pp-green"><strong>new attachment</strong> in %1$s "%2$s":</span> %3$s', 'presspermit'), __(ucwords($row->post_type)), $row->post_title, $file_url);
                                }

                                wp_insert_attachment($att);
                            } else {
                                if ($echo) {
                                    printf(__('<span class="pp-blue">attachment OK in %1$s "%2$s":</span> %3$s', 'presspermit'), __(ucwords($row->post_type)), $row->post_title, $file_url);
                                }
                            }
                ...
                if ($echo) {
                    ...
                    printf(__("Operation complete: %s linked uploads were found in your post / page content.", 'presspermit'), $found_links);
                    echo '<br /><br />';
                    if ($num_inserted) {
                        printf(__('<strong>%s attachment records were added to the database.</strong>', 'presspermit'), $num_inserted);
                        echo '<br /><br />';
                    } elseif ($found_links) {
                        _e('All linked uploads are already registered as attachments.', 'presspermit');
                        echo '<br /><br />';
                    }
                }
            }
