...
jQuery(document).ready(function ($) {
    ...
    function PP_RecaptionButton(btnName, btnSelector, btnCaption) {
        ...
        if (!$(btnSelector).length && ('button.editor-post-publish-button' == btnSelector)) {
        ...
        if ($(btnSelector).length) {
            ...
            node.addClass('presspermit-editor-hidden').hide();

            node.after('<span class="presspermit-editor-button">' + node.clone().removeAttr('aria-disabled').show().html(btnCaption).wrap('<span>').parent().html() + '</span>');
        }
    ...
    var PP_InitializeBlockEditorModifications = function () {
        ...
        && ($('button.editor-post-save-draft').length || ($('div.publishpress-extended-post-status select option[value="_pending"]').length && ('pending' == $('div.publishpress-extended-post-status select').val() || '_pending' == $('div.publishpress-extended-post-status select').val())))
            ...
            if ($('button.editor-post-publish-panel__toggle').length) {
                if (typeof ppObjEdit.prePublish != 'undefined' && ppObjEdit.prePublish) {
