...
class PostAdmin
{
    ...
    public static function set_status_labels()
    {
        ...
        foreach ($custom_stati as $status => $status_args) {
            ...
            $sing = sprintf(__('%s <span class="count">()</span>', 'pps'), $label);
            $plur = sprintf(__('%s <span class="count">()</span>', 'pps'), $label);
        ...
        foreach ($wp_post_statuses as $status => $args) {
            ...
            if (empty($args->labels->publish)) {
                if ('approve' == $status) {
                    ...
                    $lbl = __('Mark In Progress', 'ppp');
                } else {
                    $lbl = sprintf(__('Submit as %s', 'ppp'), $label_name);
                }
