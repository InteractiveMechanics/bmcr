...
class Workflow {
    ...
    public static function getNextStatusObject($post_id = 0, $args = [])
    {
        ...
        $defaults = ['moderation_statuses' => [], 'can_set_status' => [], 'force_main_channel' => false];
        ...
        $default_by_sequence = $pp->getOption('moderation_statuses_default_by_sequence') && !PWP::isBlockEditorActive($post->post_type);
        ...
        && (!$default_by_sequence || apply_filters('presspermit_editor_default_publish', false, $post))
            ...
            if (!$moderation_statuses) {
                ...
                foreach ($moderation_statuses as $_status_obj) {
                    if (!empty($can_set_status[$_status_obj->name])) {
            ...
            if (($post_status_obj->name == $post->post_status) && current_user_can($type_obj->cap->publish_posts)) {
