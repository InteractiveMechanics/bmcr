 ...
 * Description: Your custom post statuses registered and implemented. Workflow statuses (also requires PP Collaborative Editing) allow unlimited steps between pending and published, each with distinct capability requirements and role assignments.
 * Author:      PublishPress
...
if (!defined('PRESSPERMIT_STATUSES_FILE')) {
    ...
    add_action(
        ...
        function()
        {
            ...
            if (is_admin()) {
                @load_plugin_textdomain('pps', '', dirname(plugin_basename(__FILE__)) . '/languages');

                $title = __('Status Control', 'pps');
            } else {
